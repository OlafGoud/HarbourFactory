...
public class AStarSearch {
	...
	private static final int ROW = (800/30)+2;
	private static final int COL = (600/30)+2;
	private static ArrayList<String> list;

	public static ArrayList<String> getPath(Location src, Location dest) {
		// Description of the Grid-
		// 1--> The cell is not blocked
		// 0--> The cell is blocked
		JSONObject saveFileTiles = HarbourFactoryMain.map;
		...
		while(it.hasNext()) {
			...
			if(tile.getString("material").equals("weg")) {
				grid[row][col] = 1;
			} else {
				grid[row][col] = 0;
			}
		...
		int[] scr1 = {src.getX() , src.getY()};
		int[] dest1 = {dest.getX() , dest.getY()};

		aStarSearch(grid, scr1, dest1);
		return list;
	}
	...
	private static void tracePath(Cell[][] cellDetails, int[] dest) {
		...
		path.put(new int[]{row, col}, true);
		ArrayList<int[]> pathList = new ArrayList<>(path.keySet());
